...
from _src.models import GloNet, GloNet10, GloNet24, GloNet50, GloNet100, GloNet200, ResNet10, ResNet24, ResNet50, ResNet100, ResNet200, MLP
...
def create_model(
    ...
    elif flags.model == "MLP": return MLP(features=flags.features, num_layers=flags.num_layers, output_dim=1)
    elif flags.model == "GloNet10": return GloNet10(features=flags.features)
    ...
    elif flags.model == "ResNet10": return ResNet10(features=flags.features)
...
def compute_l1_norms_leaves(norm_params, ordered_keys: list) -> list:
    ...
    for k, v in ordered_norm_params.items():
        if "bias" in v: # is a layer
            ...
            for layer in ["Dense_0", "BatchNorm_0", "Dense_1", "BatchNorm_1"]:
                vv = v[layer]
                values.append(vv["bias"] + vv["kernel"] if "kernel" in vv else vv["bias"] + vv["scale"])
